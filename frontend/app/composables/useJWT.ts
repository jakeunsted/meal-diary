/**
 * Client-side JWT utility for checking token expiration
 * Browser-compatible version (doesn't use Node.js Buffer)
 */
...
export function decodeJWT(token: string): { exp?: number; iat?: number; [key: string]: any } | null {
  try {
    ...
    console.error('[JWT Client] Failed to decode token:', error);
    return null;
  }
}
